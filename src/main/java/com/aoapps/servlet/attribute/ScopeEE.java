...
public abstract class ScopeEE<C> extends com.aoapps.lang.attribute.Scope<C> {
	...
	public static final class Page extends ScopeEE<JspContext> {
		...
		public static int getScopeId(String scope) throws LocalizedIllegalArgumentException {
			if(scope == null || scope.isEmpty() || SCOPE_PAGE.equals(scope)) return PageContext.PAGE_SCOPE;
			else if(SCOPE_REQUEST.equals(scope)) return PageContext.REQUEST_SCOPE;
			else if(SCOPE_SESSION.equals(scope)) return PageContext.SESSION_SCOPE;
			else if(SCOPE_APPLICATION.equals(scope)) return PageContext.APPLICATION_SCOPE;
			else throw new LocalizedIllegalArgumentException(RESOURCES, "Page.scope.invalid", scope);
		}
